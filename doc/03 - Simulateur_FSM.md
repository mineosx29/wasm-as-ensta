...
## Qu'est une FSM ?
Une machine à états finis est une machine qui est susceptible d'être dans un nombre fini d'états mais étant un moment donné que dans un état à la fois. L'état dans lequel il se trouve se nomme l'état courant. Le passage d'un état à un autre est activé par un événement ou une condition.
